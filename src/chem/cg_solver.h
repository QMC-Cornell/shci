...
//std::cout<<" "<<hamiltonian_matrix.rows[i].get_index(j_id)<<" "<<Hcc_vals[i][j_id];
      }
    }
...
std::cout<<"solver initialized"<<std::endl;
    ...
    while (r.norm() > 1e-10) {
      ...
      double old_r_norm_sqr = std::pow(r.norm(), 2);
      std::cout<<"r.norm "<<r.norm()<<std::endl;
      double alpha = old_r_norm_sqr / (p.dot(Ap));
      ans += alpha * p;
      r -= alpha * Ap;
      double beta = std::pow(r.norm(), 2) / old_r_norm_sqr;
      p *= beta;
      p += r;
    }
//std::cout<<"\n"<<ans; std::exit(0);
