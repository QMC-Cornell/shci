...
class Hamiltonian {
 ...
 private:
  ...
  std::unordered_map<std::string, size_t> alpha_to_alpha_id;

  std::unordered_map<std::string, size_t> beta_to_beta_id;
...
template <class S>
void Hamiltonian<S>::update_abdet() {
...
template <class S>
void Hamiltonian<S>::update_abm1() {
  std::unordered_set<size_t> updated_alphas;
  std::unordered_set<size_t> updated_betas;
  auto& det = tmp_dets[0];
...
template <class S>
void Hamiltonian<S>::update_absingles() {
  std::unordered_set<size_t> updated_alphas;
  std::unordered_set<size_t> updated_betas;
...
#pragma omp parallel for schedule(static, 1)
  for (size_t alpha_id = 0; alpha_id < n_unique_alphas; alpha_id++) {
    ...
    const auto& half_det = hps::parse_from_string(alpha);
...
#pragma omp parallel for schedule(static, 1)
  for (size_t beta_id = 0; beta_id < n_unique_betas; beta_id++) {
    ...
    const int thread_id = omp_get_thread_num();
    const auto& half_det = hps::parse_from_string(beta);
...
template <class S>
void Hamiltonian<S>::update_matrix() {
...
#pragma omp parallel for schedule(static, 1)
  for (size_t det_id = proc_id; det_id < n_dets; det_id += n_procs) {
    ...
    const auto& det = hps::parse_from_string(system.get_det(det_id));
    Det connected_det;
    ...
    for (auto it = alpha_dets.begin(); it != alpha_dets.end(); it++) {
      ...
      hps::parse_from_string(system.get_det(alpha_det_id));
    ...
    for (auto it = beta_dets.begin(); it != beta_dets.end(); it++) {
      ...
      hps::parse_from_string(system.get_det(beta_det_id));
      const double H = system.get_hamiltonian_elem(det, connected_det);
      if (std::abs(H) < Util::EPS) continue;
    ...
    for (const auto alpha_single : alpha_singles) {
      ...
      for (auto it = beta_singles.begin(); it != beta_singles.end(); it++) {
        ...
        if (related_beta_ids[ptr] == beta_single) {
          ...
          hps::parse_from_string(system.get_det(related_det_id));
          const double H = system.get_hamiltonian_elem(det, connected_det);
          if (std::abs(H) < Util::EPS) continue;
          row.append(beta_det_id, H);
        }
...
void Hamiltonian<S>::sort_by_first(std::vector<size_t>& vec1, std::vector<size_t>& vec2) {
  ...
  std::sort(
      vec.begin(), vec.end(), [&](const auto& a, const auto& b) { return a.first < b.first; });
  for (size_t i = 0; i < n_vec; i++) {
    vec1[i] = vec[i].first;
    vec2[i] = vec[i].second;
  }
