...
* `eps_vars_schedule`: an array of additional variational epsilons run beforehand for a better selection of determinants.
* `target_error`: target error for stochastic perturbation, default to 1.0e-5.
* `var_only`: only run variation.
* `for_var`: run variation even if valid wavefunction files already exists.
...
* `point_group`: supports `dih`, `d2h`, `c2v` or `none`.
* `irreps`: an array of irreducible representations.
* `irrep_occs_up` and `irrep_occs_dn`: occupation of each irreducible representation for up and down electrons respectively.
